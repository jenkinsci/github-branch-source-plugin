...
public class GitHubRepoSizeEstimator {
  /**
   * This extension intends to perform a GET request without any credentials on the provided
   * repository URL to return the size of repository.
   */
  @Extension
  public static class RepositorySizeGithubAPI extends GitToolChooser.RepositorySizeAPI {
    ...
    public boolean isApplicableTo(String repoUrl, Item context, String credentialsId) {
      StandardCredentials credentials =
          Connector.lookupScanCredentials(context, repoUrl, credentialsId);
      try {
    ...
    @Override
    public Long getSizeOfRepository(String repoUrl, Item context, String credentialsId)
        throws Exception {
      StandardCredentials credentials =
          Connector.lookupScanCredentials(context, repoUrl, credentialsId);
      GitHubRepositoryInfo info = GitHubRepositoryInfo.forRepositoryUrl(repoUrl);
